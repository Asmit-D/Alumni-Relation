      ...
      <div className='flex flex-col  rounded-t-xl mx-15 pt-8 mb-2'>
        ...
        <div className='flex flex-row flex-wrap justify-between bg-white/20 border-t border-stone-800 px-4 py-2'>
          ...
          {choices.map((choice, index) => (
            ...
            onPressedChange={() => handleToggleChange(choice)}
            className='text-zinc-200 rounded-full active:scale-[.97] duration-200 bg-white/15 place-self-center h-fit m-1 py-1 px-3'>{choice}</Toggle>
          ))}
          </div>
          <div>
            <Button onClick={() => setSelectedChoices([])} className='text-zinc-200 bg-white/15 rounded-full active:scale-[.97] duration-200 h-fit mx-1 py-1 px-3'>Clear</Button>
          </div>
        </div>

        {/* Alumni list */}
        <ul>
          {!Alumni ? <p className="text-muted-foreground">Alumni not found</p> : 
          <div>
            ...
            (typeof alumni.domains === 'string' 
              ? selectedChoices.includes(alumni.domains)
              : alumni.domains.some(domain => selectedChoices.includes(domain))
            ))
          ).map((alumni) => (
              ...
              <li className="flex items-center bg-gray-900 active:bg-white/10 active:scale-[.994] hover:bg-white/20 duration-200 border border-stone-800 p-4">
                <div className="rounded-full flex-none overflow-hidden text-zinc-200">
                  <Avatar className="h-16 w-16 md:h-20 md:w-20">
                    {alumni.dp ? 
                    <div>
                    <AvatarImage src={`http://127.0.0.1:8000/${alumni.dp}`} className="object-cover" alt={alumni.name} />
                    <AvatarFallback className=""> <UserRound size={30} /> </AvatarFallback>
                    </div> : 
                    <AvatarFallback className=""> <UserRound size={30} /> </AvatarFallback>
                    }
                  </Avatar>                
